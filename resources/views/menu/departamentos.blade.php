...
<nav class="navbar navbar-expand-lg">
    ...
    <section class="contenedor-viajes">
        <div class="titulo-seccion">
            <h1>Items Principales Destinos Del Peru</h1>
        </div>
        <hr>
        <article>
                    ...
                    <div class="card-body">
                        <div class="card-titulo-content">
                            ...
                            <span class="message green">Aun disponible</span> <span
                                class="message red">{{ $item->estado }}
                            </span>
                        </div>
                        <p class="card-text">{{ $item->descripcion }}</p>
                        <form action="{{route('departamento.eliminar', $item->id)}}" method="post">
                            @csrf
                            @method('delete')
                            <input type="submit" style="float: left;" value="Eliminar" class="btn btn-danger"/>
                        </form>
                        <a href="{{route('departamento.editar', $item->id)}}" class="btn btn-warning col-6 float-end">Editar</a>
                    </div>
                </div>
            @endforeach
        </article>
