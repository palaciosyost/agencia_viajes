...
@section('titulo')
    ...
    <title>{{$editar->nombre}} Editar</title>
@endsection
    ...
    <div class="container">
        <div class="w-75 mt-5">
            <div class="titulo-form">
                <h1>Actualiza el registro de {{$editar->nombre}}</h1>
            </div>
            <hr>
            <form action="{{ route('departamento.actualizar', $editar->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-3 row">
                    <label for="staticEmail" class="col-sm-2 col-form-label fonw">Nombre</label>
                    <div class="col-sm-10">
                        <input type="text" name="nombre" class="form-control" placeholder="nombre" value="{{$editar->nombre}}">
                        @error('nombre')
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="descripcion"rows="5" class="form-control">{{$editar->descripcion}}</textarea>
                        @error('descripcion')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="estado" class="form-control" accept="image/*" placeholder="Estado" value="{{$editar->estado}}">
                        @error('estado')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
                <input type="submit" value="Guardar Registro" class="btn btn-warning ">
            </form>
